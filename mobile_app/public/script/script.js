...
for (let i = 1; i <= data.scenes.length; i++) {
    ...
    aScene.style.top = (parseInt(data.scenes[i - 1].y.slice(0,data.scenes[i - 1].y.length-2)) * 1.65) + "px";
    aScene.style.left = (parseInt(data.scenes[i - 1].x.slice(0,data.scenes[i - 1].x.length-2)) * 1.65) + "px";
